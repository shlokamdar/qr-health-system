...
import { FileText, ChevronRight, X, Calendar, User, ExternalLink, Download, FileCheck, Info, FileX, FlaskConical } from 'lucide-react';
import { getFileUrl } from '../../utils/api';

const MedicalRecordList = ({ records, labReports = [] }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-[10px] border border-[#E2E8F0] p-6 shadow-sm">
                ...
                {records.length === 0 ? (
                    <div className="py-8 text-center">
                        <FileX className="w-10 h-10 text-[#9CA3AF] mx-auto mb-2" />
            ...
            <div className="bg-white rounded-[10px] border border-[#E2E8F0] p-6 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                    <h3 className="text-[16px] font-bold text-[#0D1B2A] flex items-center gap-2">
                        <FlaskConical className="w-5 h-5 text-[#2EC4A9]" />
                        <span>Lab Reports</span>
                    </h3>
                    <span className="text-[#9CA3AF] text-[13px]">{labReports.length} reports available</span>
                </div>

                {labReports.length === 0 ? (
                    <div className="py-8 text-center">
                        <FlaskConical className="w-10 h-10 text-[#9CA3AF] mx-auto mb-2" />
                        <p className="text-[#9CA3AF] text-[14px]">No lab reports found.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {labReports.map(rep => (
                            <div
                                ...
                                className="flex items-center gap-4 p-4 border border-[#E2E8F0] rounded-[8px] hover:border-[#2EC4A9] hover:bg-[#2EC4A9]/5 cursor-pointer transition-all group"
                            >
                                <div className="w-10 h-10 bg-[#F0FDF9] rounded-[6px] flex items-center justify-center shrink-0">
                                    <FlaskConical className="w-5 h-5 text-[#2EC4A9]" />
                                </div>
                                <div className="min-w-0">
                                    <p className="text-[#0D1B2A] font-bold text-[14px] truncate">
                                        {rep.test_type?.name || rep.test_type_name || 'Lab Report'}
                                    </p>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[#9CA3AF] text-[12px]">
                                            {rep.technician?.lab?.name || rep.lab_name || 'Diagnostic Lab'}
                                        </span>
                                        <span className="text-[#9CA3AF] text-[12px]">•</span>
                                        <span className="text-[#9CA3AF] text-[12px]">
                        ...
                        <div className="p-10 space-y-8 overflow-y-auto custom-scrollbar">
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-2xl font-black text-[#0D1B2A] leading-tight">
            ...
            {selectedLabReport && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 bg-[#0D1B2A]/60 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-xl w-full overflow-hidden border border-white/20 animate-in zoom-in-95 slide-in-from-bottom-4 duration-500 flex flex-col">
                        <div className="bg-[#2EC4A9] p-8 relative overflow-hidden shrink-0">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-[80px] -mr-32 -mt-32" />
                            <div className="relative z-10 flex justify-between items-center text-white">
                                <div className="flex items-center gap-4">
                                    <div className="p-3 bg-white/20 backdrop-blur-md rounded-2xl text-white">
                                    ...
                                    <div>
                                        <span className="text-white/60 text-[10px] font-black uppercase tracking-widest">DIAGNOSTIC REPORT</span>
                                        <h3 className="font-black text-2xl tracking-tight leading-none mt-1">Lab Report</h3>
                                    </div>
                                </div>
                                <button
                        ...
                        <div className="p-10 space-y-8 overflow-y-auto custom-scrollbar">
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-2xl font-black text-[#0D1B2A] leading-tight">
                                        {selectedLabReport.test_type?.name || selectedLabReport.test_type_name || 'Lab Report'}
                                    </p>
                                </div>

                                {selectedLabReport.comments && (
                                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block flex items-center gap-1.5">
                                            <Info className="w-3 h-3" />
                                            Technician Notes / Key Findings
                                        </label>
                                        <p className="text-slate-600 font-medium leading-relaxed">
                                            {selectedLabReport.comments}
                                        </p>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-5 rounded-2xl bg-white border border-slate-100">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Report Date</label>
                                        <div className="flex items-center gap-2 text-[#0D1B2A] font-bold">
                                            <Calendar className="w-4 h-4 text-[#2EC4A9]" />
                                            <span>{new Date(selectedLabReport.created_at).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                    <div className="p-5 rounded-2xl bg-white border border-slate-100">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Lab</label>
                                        <div className="flex items-center gap-2 text-[#0D1B2A] font-bold">
                                            <FlaskConical className="w-4 h-4 text-[#2EC4A9]" />
                                            <span className="truncate">
                                                {selectedLabReport.technician?.lab?.name || selectedLabReport.lab_name || 'Diagnostic Lab'}
                                            </span>
                                        </div>
                                    </div>
                                </div>

                                {selectedLabReport.file && (
                                    <div className="pt-4">
                                        <a
                                            href={getFileUrl(selectedLabReport.file)}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="group flex items-center justify-between bg-[#0D1B2A] text-white p-6 rounded-[2rem] hover:bg-[#1a2e41] transition-all shadow-xl shadow-[#0D1B2A]/10 active:scale-[0.98]"
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="font-black leading-none">View Report File</p>
                                                    <p className="text-white/40 text-[10px] font-black uppercase tracking-widest mt-1.5">PulseID Secure Portal</p>
                                                </div>
                                            </div>
                                            <ExternalLink className="w-5 h-5 text-white/40 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
